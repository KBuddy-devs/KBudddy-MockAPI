...
class CommentSchema(BaseModel):
    ...
    userId: UUID = Field(
        ..., title="User ID", description="댓글 작성자의 고유 식별자입니다."
    )
...
class QnALike(BaseModel):
    userId: UUID = Field(
        ..., title="User ID", description="사용자의 고유 식별자입니다."
    )
...
class QnASchema(BaseModel):
    ...
    likeCount: int = Field(..., title="Like Count", description="좋아요 수입니다.")
    likes: List[QnALike] = Field(
        None, title="Likes", description="좋아요를 누른 사용자 목록입니다."
    )
...
class QnACreate(BaseModel):
    ...
    description: str = Field(..., title="Description", description="QnA의 내용입니다.")
    file: List[FileSchema] = Field(
        ..., title="Files", description="첨부된 파일 목록입니다."
    )


class QnAUpdate(BaseModel):
    categoryId: Optional[int] = Field(
        None, title="Category ID", description="카테고리의 고유 식별자입니다."
    )
    title: Optional[str] = Field(None, title="Title", description="QnA의 제목입니다.")
    description: Optional[str] = Field(
        None, title="Description", description="QnA의 내용입니다."
    )
    file: Optional[List[FileSchema]] = Field(
        None, title="Files", description="첨부된 파일 목록입니다."
    )
